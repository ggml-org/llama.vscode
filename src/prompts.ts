...
export class Prompts {
    ...
    FIM_CODE_SNIPPET = `You are an expert code completion engine. Provide high-quality, relevant code snippets or full blocks of code to complete the user's intent. Focus on completing the most logical next steps in the code.\n\nGiven the surrounding code, complete the missing part. Respond with only the code.\n\n<|fim_prefix|>{inputPrefix}{prompt}<|fim_suffix|>{inputSuffix}<|fim_middle|>`
    EXPLAIN_CODE_PROMPT = `Explain the following code. Focus on its purpose, how it works, and any important design patterns or algorithms used. Provide a concise explanation with clear examples if necessary.\n\nCode:\n\`\`\`\n{code}\`\`\`\n\nExplanation:`
    GENERATE_CODE_PROMPT = `You are an expert code generator. Generate a complete and well-structured code block (function, class, or snippet) based on the following description and context. Include necessary imports and comments. Respond with only the code.\n\nDescription:\n{description}\n\nContext from project files:\n{rag_context}\n\nGenerated Code:`
    FIX_LINTER_PROMPT = `The following code has linter errors. Please provide the corrected code. Only output the corrected code, no explanations.\n\nLinter Errors:\n{linter_errors}\n\nOriginal Code:\n\`\`\`\n{original_code}\n\`\`\`\n\nCorrected Code:`
    GENERATE_UNIT_TEST_PROMPT = `You are an expert in writing unit tests. Generate unit tests for the following code snippet. Use a popular testing framework (e.g., Jest for JavaScript/TypeScript, JUnit for Java, Pytest for Python). Focus on covering common scenarios and edge cases. Only output the test code, no explanations.\n\nCode to test:\n\`\`\`\n{code_to_test}\n\`\`\`\n\nAdditional Instructions:\n{instructions}\n\nGenerated Unit Tests:`
    ANALYZE_ERROR_PROMPT = `Analyze the following error message or stack trace. Explain the probable cause of the error and suggest potential solutions or debugging steps. Provide a clear and concise explanation.\n\nError/Stack Trace:\n\`\`\`\n{error_text}\n\`\`\`\n\nAnalysis and Suggestions:`
